...
public class DMADalekRegistry {
	public static Map<DalekType, List<String>> DMA_DALEK_TYPES = new HashMap();
	public static Map<ResourceLocation, List<IDalek>> DMA_DALEK_SPAWNS = new HashMap();
	...
	private static void register(String registryKey, IDalek dalek) {
		...
		if (!DMA_DALEK_TYPES.containsKey(dalek.getType())) {
			DMA_DALEK_TYPES.put(dalek.getType(), new ArrayList());
		}

		((List)DMA_DALEK_TYPES.get(dalek.getType())).add(dalek.getID());
	}

	public static IDalek getRandomDalek(Random rand, DalekType type) {
		List<String> dalekList = (List)DMA_DALEK_TYPES.get(type);
	...
	public static IDalek getDalekForBiome(World world, Biome biomeIn) {
		if (DMA_DALEK_SPAWNS.containsKey(biomeIn.getRegistryName())) {
			List<IDalek> daleks = (List)DMA_DALEK_SPAWNS.get(biomeIn.getRegistryName());
	...
	public static void addSpawn(IDalek dalek, RegistryKey<Biome>... biomes) {
		...
		for(int var4 = 0; var4 < var3; ++var4) {
			...
			if (DMA_DALEK_SPAWNS.containsKey(biome.location())) {
				((List)DMA_DALEK_SPAWNS.get(biome.location())).add(dalek);
			} else {
				List<IDalek> l = new ArrayList();
				l.add(dalek);
				DMA_DALEK_SPAWNS.put(biome.location(), l);
			}
		}
	...
	public static void addSpawn(RegistryKey<Biome> biome, IDalek... daleks) {
		...
		for(int var5 = 0; var5 < var4; ++var5) {
			...
			if (DMA_DALEK_SPAWNS.containsKey(b.getRegistryName())) {
				((List)DMA_DALEK_SPAWNS.get(b.getRegistryName())).add(dalek);
			} else {
				List<IDalek> l = new ArrayList();
				l.add(dalek);
				DMA_DALEK_SPAWNS.put(b.getRegistryName(), l);
			}
		}
