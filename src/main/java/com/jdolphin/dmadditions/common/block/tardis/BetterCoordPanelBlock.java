...
public class BetterCoordPanelBlock extends CoordPanelBlock implements IBetterPanel {
	...
	protected String formatIncrementMessage(Boolean add, Direction.Axis axis, CoordPanelTileEntity tile, TardisFlightData flightData) {
		return (add ? "Added " : "Subtracted ") + tile.incrementValue + (add ? " to " : " from ")
			+ axis.toString().toUpperCase() + " ("
			+ flightData.getPos(axis)
			+ ")";
	}
						...
						if (flightData != null) {
							switch (buttonClicked) {
								case ADD_X:
									...
									ChatUtil.sendMessageToPlayer(player, this.formatIncrementMessage(true, Direction.Axis.X, tet, flightData), ChatUtil.MessageType.STATUS_BAR);
									break;
								case ADD_Y:
									flightData.incrementPos(tet.incrementValue, Direction.Axis.Y);
									ChatUtil.sendMessageToPlayer(player, this.formatIncrementMessage(true, Direction.Axis.Y, tet, flightData), ChatUtil.MessageType.STATUS_BAR);
									break;
								case ADD_Z:
									flightData.incrementPos(tet.incrementValue, Direction.Axis.Z);
									ChatUtil.sendMessageToPlayer(player, this.formatIncrementMessage(true, Direction.Axis.Z, tet, flightData), ChatUtil.MessageType.STATUS_BAR);
									break;
								case SUB_X:
									flightData.incrementPos(-tet.incrementValue, Direction.Axis.X);
									ChatUtil.sendMessageToPlayer(player, this.formatIncrementMessage(false, Direction.Axis.X, tet, flightData), ChatUtil.MessageType.STATUS_BAR);
									break;
								case SUB_Y:
									flightData.incrementPos(-tet.incrementValue, Direction.Axis.Y);
									ChatUtil.sendMessageToPlayer(player, this.formatIncrementMessage(false, Direction.Axis.Y, tet, flightData), ChatUtil.MessageType.STATUS_BAR);
									break;
								case SUB_Z:
									flightData.incrementPos(-tet.incrementValue, Direction.Axis.Z);
									ChatUtil.sendMessageToPlayer(player, this.formatIncrementMessage(false, Direction.Axis.Z, tet, flightData), ChatUtil.MessageType.STATUS_BAR);
									break;
								case INCREMENT:
	...
	public CoordPanelButtons getButton(double mouseX, double mouseY, double mouseZ, Direction facing, AttachFace face) {
		...
		while (buttonsIterator.hasNext()) {
			CoordPanelButtons button = buttonsIterator.next();
