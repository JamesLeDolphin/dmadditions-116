...
public class CitadelStructure extends Structure<NoFeatureConfig> {
	...
	public StructureStart<?> generate(DynamicRegistries registries, ChunkGenerator generator, BiomeProvider biomeProvider,
									  TemplateManager manager, long seed, ChunkPos chunkPos, Biome biome, int i, SharedSeedRandom seedRandom,
									  StructureSeparationSettings separationSettings, NoFeatureConfig featureConfig) {

		if (DMACommonConfig.generateCitadel()) {
			BlockPos pos1 = new BlockPos(-4320, 0, -4800);
			BlockPos pos = chunkPos.getWorldPosition();
			if (Objects.requireNonNull(biome.getRegistryName()).toString().contains("dmadditions:gallifrey")) {
				if (pos.equals(pos1)) {
		...
		return new Start(this, chunkPos.x, chunkPos.z,
			MutableBoundingBox.createProper(0, 0, 0, 410, 201, 205),
			i, seed);
	}
